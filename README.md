...
For the deployed frontend, set its API base URL to the deployed backend URL followed by `/api` (for example, `https://api.example.com/api`). Set `APP_URL` on the backend to that same backend URL so the `image_url` values returned by the public endpoints point to the deployed media route.
